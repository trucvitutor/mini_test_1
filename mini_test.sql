...
select  count(Students.id) as thong_ke, Classes.name_ as ten_hoc_vien,Students.classes_id as id from Classes join Students on Classes.id= Students.classes_id group by Students.classes_id;
-- select address_id, count(*) as so_luong_tinh from Students group by address_id;
select Students.address_id ,Address.address as tinh_thanh,count( Students.address_id) as so_luong_sinh_vien  from Address join Students on Address.id=Students.address_id group by Students.address_id;
select avg(points.point) as avg_point, Course.`name`
from points
join Course on Points.course_id = course.id
